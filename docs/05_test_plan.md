...
| A4 | `IsHoliday` khớp nhau giữa train/test và features khi join | `test_join_integrity.py::test_isholiday_consistency` | `tests/test_ingestion/` |
...
| A7 | MarkDown NaN được hiểu là "không có khuyến mãi" qua flag `has_markdown`, không fillna(0) mù quáng | `test_markdown_flag.py` | `tests/test_features/` |
...
| A36 | Forward-fill CPI/Unemployment chỉ áp dụng cho dòng thực sự NaN sau join (không ghi đè giá trị thật đã có) | `test_macro_forward_fill.py::test_forward_fill_does_not_overwrite_existing_values` | `tests/test_features/` |
...
| A38 | Flag `cpi_is_forward_filled`/`unemployment_is_forward_filled` chỉ True đúng ở các dòng đã được điền, False ở mọi dòng khác (kể cả dòng NaN không điền được vì không có giá trị lịch sử nào) | `test_macro_forward_fill.py::test_forward_fill_flag_matches_actually_filled_rows` | `tests/test_features/` |
| A39 | `aggregate_to_store_date` SUM đúng Weekly_Sales theo (Store,Date); IsHoliday không nhất quán trong cùng (Store,Date) phải raise `DataContractError`, không âm thầm `.first()` sai | `test_aggregate_to_store_date.py::test_aggregate_sums_weekly_sales_across_dept`, `test_isholiday_inconsistency_raises` | `tests/test_ingestion/` |
