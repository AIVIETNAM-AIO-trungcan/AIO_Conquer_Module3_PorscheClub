...
| A53 | `add_horizon_targets` sinh đúng `target_t+{h}` = Weekly_Sales sau đúng h tuần CÙNG group (Store); dòng cuối mỗi group → NaN đúng, không lấy nhầm từ group khác; hàm thuần, tự sort, không sửa df gốc | `test_horizon_target.py` | `tests/test_features/` |
| A54 | `DirectMultiHorizonModel.fit`/`.predict` đúng API multi-target: mỗi horizon có estimator độc lập, dropna theo TỪNG horizon riêng, `.predict()` trả đúng shape `(n_rows, horizon)`, `.predict_horizon()` khớp cột tương ứng trong `.predict()` đầy đủ | `test_direct_multihorizon.py` | `tests/test_models/` |
| A55 | `wape()` tính đúng công thức có `abs()` ở mẫu số — phân biệt rõ kết quả đúng/sai khi `y_true` chứa số âm (khác lỗi thiếu abs đã phát hiện trong `direct_multimodel_rf.ipynb`) | `test_metrics_correctness.py::test_wape_correct_formula_with_abs_in_denominator` | `tests/test_evaluation/` |
| A56 | Pipeline `run_train_multistep.py` (luồng build_feature_matrix → split → horizon targets → fit/predict/metric) chạy hết không crash trên fixture đủ lớn, sinh bảng metric đủ cột; horizon xa với ít dòng VALID bị bỏ qua an toàn; horizon không còn dòng TRAIN nào sau dropna phải raise `ValueError` rõ ràng (không để sklearn ném lỗi khó hiểu) | `test_run_train_multistep_end_to_end.py` | `tests/test_pipelines/` |

> Chi tiết đầy đủ skeleton của A16–A20 (Conformal Prediction): `08_uncertainty_conformal.md` mục 6. Chi tiết đầy đủ skeleton của A21–A23 (Dashboard): `07_dashboard_spec.md` mục 6. Chi tiết đầy đủ A25–A35 (Run tracking/versioning): `tests/test_utils/test_run_tracking.py`, `docs/00_decisions.md`. Chi tiết A36–A38 (Macro forward-fill), A39–A45 (Đổi đơn vị dự báo + features.yaml runtime) và A46 (Buffer nối train/valid): `docs/00_decisions.md` [2026-08-19]. Chi tiết A47–A52 (Đồng bộ xử lý dữ liệu theo notebook — aggregate/join theo IsHoliday, MarkDown fillna(0), macro ffill 4 cột, macro lag52 cho valid, split theo tỷ lệ 2/3, lags/rolling_windows từ config): `docs/00_decisions.md` [2026-08-31] "Đồng bộ xử lý dữ liệu theo notebooks/01. Preprocessing.ipynb". Chi tiết A53–A56 (Direct multi-step HORIZON=10 — horizon target, DirectMultiHorizonModel, wape(), pipeline run_train_multistep.py): `docs/00_decisions.md` [2026-08-31] "Direct multi-step HORIZON=10, không chia nhóm".
